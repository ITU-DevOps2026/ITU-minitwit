...
To mitigate this error, we should have moved our Prometheus container to the Manager droplet, and then from here it would be able to scrape all the individual replicas, and find averages over these, via the Docker network, and communicate this to Grafana on the monitoring droplet. However, this would still be a problem due to the workers being on different droplets than the manager, therefore we would need a package that could discover the exact location of each replica, for example this package: [HERE](https://github.com/stuckyhm/prometheus-docker-sd).

In addition to this we also wanted to monitor all of our services, i.e. not "just" the webservice, but also the database, to make it easier to determine where a error/bottleneck is located, however we did not get to implement this.

Due to us developers not wanting to constantly monitor our system, we have set up a *Uptime Check*, that monitors our droplets, and gives everyone in the team a notification if:
* https://bigtwit.app is down
* there is latency over 1000ms when accessing https://bigtwit.app 
* The SSL certificate of bigtwit.app is about to expire.(However Traefik should automatically renew this certificate, before they expire.)
...
### Dashboards
To see the dashboard, go to [46.101.69.11:3000](http://46.101.69.11:3000) , where the dashboard + logs can be viewed. The credentials is specified in the post made by Helge.

We made 2 different dashboards, one made with a developer focus(Infrastructure Health), and one made for a Business stakeholder focus(Product Performance).

#### Minitwit Infrastructure Health, dashboard
![The first part of the Minitwit Infrastructure Health dashboard](../images/dashboard_infrastructure_1.png)
![The second part of the Minitwit Infrastructure Health dashboard](../images/dashboard_infrastructure_2.png)
![The third part of the Minitwit Infrastructure Health dashboard](../images/dashboard_infrastructure_3.png)

This dashboard give the developers information they need like:
...
We have, in the source code, created a */healthz* endpoint, that shows when minitwit is up and running, however this is only used for spinning up the system, and our dashboard's health check is a health check of Prometheus was able to scrape the target. If we had more time, we would have used the */healthz* endpoint instead, which would give a more relvant answer, i.e. seperated from Prometheus.

#### Minitwit Product Performance, dashboard
![The first part of the Minitwit Infrastructure Health dashboard](../images/dashboard_product_1.png)
![The second part of the Minitwit Infrastructure Health dashboard](../images/dashboard_product_2.png)
![The third part of the Minitwit Infrastructure Health dashboard](../images/dashboard_product_3.png)
