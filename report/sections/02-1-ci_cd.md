...
![Simplified CI/CD pipeline with no individual workflow steps shown](./images/overall_cicd_pipeline.png){width=70%}

In figure XX is an illustration of the automated testing workflow. This workflow is reused in both the workflow for deployment to test environment and the one for deployment to production environment. 
...
The workflow for deploying to test environment is very similar to figure XX, an illustration is included in appendix 5.1. Illustrations of the workflows for CodeQL, SonarQube and Trivy can be found in the same place. 
