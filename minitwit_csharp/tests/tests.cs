using System.Net;

using Xunit;

/* Set tests to run sequentially as they will fail if run in parallel. 
Inspired by code found here: https://stackoverflow.com/questions/1408175/execute-unit-tests-serially-rather-than-in-parallel 
*/
namespace tests
{
  [CollectionDefinition("Non-Parallel Collection", DisableParallelization = true)]
  
  [Collection("Non-Parallel Collection")]
  public class MiniTwitTests : IDisposable
  {
    private readonly string _baseUrl = "http://localhost:5035";
    private readonly HttpClient _client;
    private readonly CookieContainer _cookieContainer;

    public MiniTwitTests()
    {
        _cookieContainer = new CookieContainer();
        var handler = new HttpClientHandler { CookieContainer = _cookieContainer };
        _client = new HttpClient(handler);
    }

    public void Dispose() => _client.Dispose();

    // --- Helpers ---

    private async Task<HttpResponseMessage> Register(string username, string password, string? password2 = null, string? email = null)
    {
        var data = new Dictionary<string, string>
        {
            { "username", username },
            { "password", password },
            { "password2", password2 ?? password },
            { "email", email ?? $"{username}@example.com" }
        };
        return await _client.PostAsync($"{_baseUrl}/register", new FormUrlEncodedContent(data));
    }

    private async Task<HttpResponseMessage> Login(string username, string password)
    {
        var data = new Dictionary<string, string>
        {
            { "username", username },
            { "password", password }
        };
        return await _client.PostAsync($"{_baseUrl}/login", new FormUrlEncodedContent(data));
    }

    private async Task<HttpResponseMessage> Register_and_login(string username, string password)
    {
      await Register(username, password);
      return await Login(username, password);
    }

    private async Task<HttpResponseMessage> Logout()
    {
      return await _client.GetAsync($"{_baseUrl}/logout");
    }

    private async Task<HttpResponseMessage> AddMessage(string text)
    {
        var data = new Dictionary<string, string> { { "text", text } };
        var response = await _client.PostAsync($"{_baseUrl}/add_message", new FormUrlEncodedContent(data));
        
        if (!string.IsNullOrEmpty(text))
        {
            var content = await response.Content.ReadAsStringAsync();
            Assert.Contains("Your message was recorded", content);
        }
        return response;
    }

    // --- Tests ---

    [Fact]
    public async Task TestRegister()
    {
      var r = await Register("user1", "default");
      Assert.Contains("You were successfully registered and can login now", await r.Content.ReadAsStringAsync());

      r = await Register("user1", "default");
      Assert.Contains("The username is already taken", await r.Content.ReadAsStringAsync());

      r = await Register("", "default");
      Assert.Contains("You have to enter a username", await r.Content.ReadAsStringAsync());
      
      r = await Register("meh", "");
      Assert.Contains("You have to enter a password", await r.Content.ReadAsStringAsync());

      r = await Register("meh", "x", "y");
      Assert.Contains("The two passwords do not match", await r.Content.ReadAsStringAsync());

      r = await Register("meh", "foo", email:"broken");
      Assert.Contains("You have to enter a valid email address", await r.Content.ReadAsStringAsync());
    }

    [Fact]
    public async Task TestLoginLogout()
    {
      var r = await Register_and_login("user1", "default");
      Assert.Contains("You were logged in", await r.Content.ReadAsStringAsync());
      r = await Logout();
      Assert.Contains("You were logged out", await r.Content.ReadAsStringAsync());
      r = await Login("user1", "wrongpassword");
      Assert.Contains("Invalid password", await r.Content.ReadAsStringAsync());

      r = await Login("user2", "wrongpassword");
      Assert.Contains("Invalid username", await r.Content.ReadAsStringAsync());
    }

    [Fact]
    public async Task TestMessageRecording()
    {
        await Register_and_login("foo", "default");
        await AddMessage("test message 1");
        await AddMessage("<test message 2>");

        var r = await _client.GetAsync($"{_baseUrl}/");
        var content = await r.Content.ReadAsStringAsync();
        
        Assert.Contains("test message 1", content);
        Assert.Contains("&lt;test message 2&gt;", content);
    }

    [Fact]
    public async Task Test_timeline()
    {
      await Register_and_login("foo", "default");
      await AddMessage("the message by foo");
      await Logout();

      await Register_and_login("bar", "default");
      await AddMessage("the message by bar");
      var r = await _client.GetAsync($"{_baseUrl}/public");
      Assert.Contains("the message by foo", await r.Content.ReadAsStringAsync());
      Assert.Contains("the message by bar", await r.Content.ReadAsStringAsync());

      r = await _client.GetAsync($"{_baseUrl}/");
      Assert.DoesNotContain("the message by foo", await r.Content.ReadAsStringAsync());
      Assert.Contains("the message by bar", await r.Content.ReadAsStringAsync());

      r = await _client.GetAsync($"{_baseUrl}/foo/follow");
      Assert.Contains("You are now following &quot;foo&quot", await r.Content.ReadAsStringAsync());

      r = await _client.GetAsync($"{_baseUrl}/");
      Assert.Contains("the message by foo", await r.Content.ReadAsStringAsync());
      Assert.Contains("the message by bar", await r.Content.ReadAsStringAsync());

      r = await _client.GetAsync($"{_baseUrl}/bar");
      Assert.DoesNotContain("the message by foo", await r.Content.ReadAsStringAsync());
      Assert.Contains("the message by bar", await r.Content.ReadAsStringAsync());

      r = await _client.GetAsync($"{_baseUrl}/foo");
      Assert.Contains("the message by foo", await r.Content.ReadAsStringAsync());
      Assert.DoesNotContain("the message by bar", await r.Content.ReadAsStringAsync());

      r = await _client.GetAsync($"{_baseUrl}/foo/unfollow");
      Assert.Contains("You are no longer following &quot;foo&quot;", await r.Content.ReadAsStringAsync());
      r = await _client.GetAsync($"{_baseUrl}/");
      Assert.DoesNotContain("the message by foo", await r.Content.ReadAsStringAsync());
      Assert.Contains("the message by bar", await r.Content.ReadAsStringAsync());
    }
  }
}
